...
def possible_operations(instruction, before, after):
    ...
    for operation in OPERATIONS:
        ...
        if op_result == after:
            result.append(operation.__name__)
    return result
...
def problem1(LINES):
    ...
    for e, experiment in enumerate(experiments):
        if len(possible_operations(*experiment)) >= 3:
            print([e, len(possible_operations(*experiment)), sorted(possible_operations(*experiment))])

    #print([experiment for experiment in experiments if len(possible_operations(*experiment)) >= 3])
...
def main():
    l = parse_input_file('test_input.txt')
    print(problem1(l))
    #print(problem2(l))
